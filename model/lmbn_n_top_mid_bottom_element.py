...
from .attention import BatchDrop, BatchFeatureErase_Top, PAM_Module, CAM_Module, SE_Module, Dual_Module, BatchFeatureErase_Top_Bottom, BatchFeatureErase_Top_Mid_Bottom_Element
...
class LMBN_n_top_mid_bottom_element(nn.Module):
    def __init__(self, args):
        super(LMBN_n_top_mid_bottom_element, self).__init__()
        ...
        self.batch_drop_block = BatchFeatureErase_Top_Mid_Bottom_Element(512, OSBlock)
    ...
    def forward(self, x):
        ...
        if self.batch_drop_block is not None:
            glo_top, glo_mid, glo_bottom, glo_element, glo = self.batch_drop_block(glo)
        ...
        glo_top = self.global_pooling(glo_top)
        glo_mid = self.channel_pooling(glo_mid)
        ...
        f_glo_top = self.reduction_4(glo_top)
        f_glo_mid = self.reduction_5(glo_mid)
        ...
        fea = [f_glo[-1], f_glo_top[-1], f_p0[-1]]

        return [f_glo_element[1], f_glo_bottom[1], f_glo[1], f_glo_top[1], f_glo_mid[1],f_p0[1], f_p1[1], f_p2[1], f_c0[1], f_c1[1]], fea, torch.stack([f_glo[0], f_glo_top[0], f_p0[0], f_p1[0], f_p2[0], f_c0[0], f_c1[0]], dim=2)
