...
from .partransformer import PartTransformer


class LMBN_n_par_3_vit(nn.Module):
    def __init__(self, args):
        super(LMBN_n_par_3_vit, self).__init__()

        self.n_ch = 2
        self.chs = 512 // self.n_ch

        self.part_transformer = PartTransformer(dim=512, heads=4, depth=1)
    ...
    def forward(self, x):
        ...
        p_head = p_head.squeeze(-1).squeeze(-1)  # (B, 512)
        p_upper = p_upper.squeeze(-1).squeeze(-1)
        p_lower = p_lower.squeeze(-1).squeeze(-1)

        part_tokens = torch.stack([p_head, p_upper, p_lower], dim=1)

        # Transformer 적용
        part_tokens = self.part_transformer(part_tokens)  # (B, 3, 512)

        # 다시 분리
        p_head, p_upper, p_lower = part_tokens[:, 0], part_tokens[:, 1], part_tokens[:, 2]  # (B, 512) each

        # BNNeck 적용 (BNNeck은 4D 입력이므로 reshape 필요)
        p_head = p_head.unsqueeze(-1).unsqueeze(-1)  # (B, 512, 1, 1)
        p_upper = p_upper.unsqueeze(-1).unsqueeze(-1)
        p_lower = p_lower.unsqueeze(-1).unsqueeze(-1)
