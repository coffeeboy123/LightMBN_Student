...
class LMBN_n_teacher_6_pam_pat(nn.Module):
    def __init__(self, args):
        super(LMBN_n_teacher_6_pam_pat, self).__init__()
    ...
    def forward(self, x):
        ...
        head  = self.pam_head(head)
        upper = self.pam_upper(upper)
        lower = self.pam_lower(lower)
        ...
        weights = torch.softmax(torch.stack([self.w_head, self.w_upper, self.w_lower]), dim=0)
        p_head = p_head * weights[0]
        p_upper = p_upper * weights[1]
        p_lower = p_lower * weights[2]
