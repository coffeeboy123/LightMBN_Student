...
class LMBN_n_teacher_6_par_3_att(nn.Module):
    def __init__(self, args):
        super(LMBN_n_teacher_6_par_3_att, self).__init__()
        ...
        self.part_weighting = nn.Sequential(
            nn.Linear(3, 6),
            nn.ReLU(),
            nn.Linear(6, 3),
            nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        # 1. 전역 평균 → scalar importance vector
        weights_input = torch.cat([
            p_head.view(p_head.size(0), -1).mean(dim=1, keepdim=True),
            p_upper.view(p_upper.size(0), -1).mean(dim=1, keepdim=True),
            p_lower.view(p_lower.size(0), -1).mean(dim=1, keepdim=True)
        ], dim=1)  # → (batch, 3)

        # 2. FC 통과

        weights = self.part_weighting(weights_input)  # (batch, 3)

        # 3. Re-weight each part feature
        p_head = p_head * weights[:, 0].view(-1, 1, 1, 1) + p_head
        p_upper = p_upper * weights[:, 1].view(-1, 1, 1, 1) + p_upper
        p_lower = p_lower * weights[:, 2].view(-1, 1, 1, 1) + p_lower
