...
class LMBN_n_fusion_5(nn.Module):
    def __init__(self, args):
        super(LMBN_n_fusion_5, self).__init__()

        self.ccsf_swap_prob = getattr(args, "ccsf_swap_prob", 0.25)
    ...
    def forward(self, x):
        ...
        c0 = cha[:, :, :, 0:1]           # (B, 512, 1, 1)
        c1 = cha[:, :, :, 1:2]           # (B, 512, 1, 1)

        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # Stochastic Partial Exchange: TRAIN 모드에서만 채널 일부 좌↔우 교환
        if self.training and self.ccsf_swap_prob > 0.0:
            B, C, _, _ = c0.shape
            # per-channel swap mask (B, C, 1, 1)
            m = (torch.rand(B, C, 1, 1, device=c0.device) < self.ccsf_swap_prob).to(c0.dtype)
            # swap
            c0, c1 = (1.0 - m) * c0 + m * c1, (1.0 - m) * c1 + m * c0
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
