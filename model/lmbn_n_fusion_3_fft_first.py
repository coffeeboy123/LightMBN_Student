...
class LMBN_n_fusion_3_fft_first(nn.Module):
    def __init__(self, args):
        super(LMBN_n_fusion_3_fft_first, self).__init__()
        ...
        self.freq_low_branch = nn.Sequential(copy.deepcopy(
            conv3), copy.deepcopy(osnet.conv4), copy.deepcopy(osnet.conv5))

        self.freq_high_branch = nn.Sequential(copy.deepcopy(
            conv3), copy.deepcopy(osnet.conv4), copy.deepcopy(osnet.conv5))
    ...
    def forward(self, x):
        ...
        X = torch.fft.rfft2(x, norm="ortho")  # (B,512,Hf,W2)
        B, C, Hf, W2 = X.shape

        # 정규화 반경 격자
        yy = torch.linspace(-1, 1, Hf, device=x.device).view(Hf,1).expand(Hf,W2)
        xx = torch.linspace(0, 1,  W2, device=x.device).view(1,W2).expand(Hf,W2)
        ...
        f_low  = torch.fft.irfft2(X_low,  s=x.shape[-2:], norm="ortho")
        f_high = torch.fft.irfft2(X_high, s=x.shape[-2:], norm="ortho")

        f_low = self.freq_low_branch(f_low)
        f_high = self.freq_high_branch(f_high)
