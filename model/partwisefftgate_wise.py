...
class PartWiseFFTGate_wise(nn.Module):
    ...
    def forward(self, p_par):
        ...
        """
        B, C, H, W = p_par.size()
        assert H == 12 and W == 1, "Input must be (B, C, 12, 1)"
        ...
        w_head_avg = F.adaptive_avg_pool2d(w_head, (1, 1))
        w_upper_avg = F.adaptive_avg_pool2d(w_upper, (1, 1))
        w_lower_avg = F.adaptive_avg_pool2d(w_lower, (1, 1))

        weights = torch.cat([w_head_avg, w_upper_avg, w_lower_avg], dim=1)  # (B, 3, 1, 1)
